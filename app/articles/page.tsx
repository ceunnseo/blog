  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <header className="mb-12">
        <h1 className="text-3xl font-bold text-white mb-2">전체 아티클</h1>
        <p className="text-gray-400">총 {rows.length}개의 글</p>
      </header>
        ...
        {rows.map((page) => {
          ...
          return (
            <Link
              ...
              className="block py-6 hover:bg-white/5 -mx-6 px-6 transition-colors"
            >
              <div className="flex items-baseline justify-between gap-6">
                <h2 className="text-lg font-medium text-white hover:text-lavender-400 transition-colors flex-1">
                  {title}
                </h2>
                <time className="text-sm text-gray-400 whitespace-nowrap font-mono">
                  {display}
                </time>
              </div>
            </Link>
